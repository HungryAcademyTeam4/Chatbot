class Message < ActiveRecord::Base
  ...
  after_create :broadcast_to_specific_channel, :broadcast_to_combined_channels
  ...
  def broadcast_to_combined_channels
    url = "http://localhost:9000/faye"
    message = { channel: "/all_messages", data: contents_hash }
    Net::HTTP.post_form(URI.parse(url), message: message.to_json)
  end
end
